...
$execute if score $(teamname) tgttos_finish_count = $(teamname) tgttos_player_count run execute unless score $(teamname) tgttos_config matches 1 run function mmt_tgttos:game_process/one_use/complete_action/team_score with storage mcmmt:tgttos teamcheck
