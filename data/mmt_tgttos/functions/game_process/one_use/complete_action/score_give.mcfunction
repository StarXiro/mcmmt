...
execute if score reward tgttos_config matches 1.. run tellraw @s {"translate":"[%s%s%s] %s%s%s","with":[{"text":"+","color":"gold"},{"score":{"name":"reward","objective":"tgttos_config"},"color":"gold"},{"text":"分","color":"gold"},{"text":"你是第","color":"gray"},{"score":{"name":"all","objective":"tgttos_finish_count"},"color":"aqua"},{"text":"个完成的玩家！","color":"gray"}]}