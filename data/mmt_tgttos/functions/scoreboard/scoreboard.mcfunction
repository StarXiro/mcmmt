...
execute store result storage mcmmt:tgttos scoreboard.tgttos_player_count int 1 run scoreboard players get all tgttos_player_count
execute store result storage mcmmt:tgttos scoreboard.tgttos_finish_count int 1 run scoreboard players get all tgttos_finish_count

#get map_name
data modify storage mcmmt:tgttos scoreboard.map_name set from entity @e[scores={tgttos_config=1},tag=map,limit=1] CustomName

#run 宏function
function mmt_tgttos:scoreboard/scoreboardextra with storage mcmmt:tgttos scoreboard